...
function AsideNavigation({pageLayout, language}){
  return (
    <div className="card-navigation">
      {
        (pageLayout.aside_preview ) && (<button className={`btn btn-secondary left-margin-2px ${panelView == 'preview' &&   'active'}`} onClick={(e) => { e.preventDefault(); setPanelView('preview')   }}>{(language === 'en_us') ? 'Preview Form' : 'Prévia do Form'}</button>)
        (pageLayout.aside_schema  ) && (<button className={`btn btn-secondary left-margin-2px ${panelView == 'schema' &&    'active'}`} onClick={(e) => { e.preventDefault(); setPanelView('schema')    }}>{(language === 'en_us') ? 'RJSF Schema' : 'RJSF Schema'}</button>)
      }
    </div>
  )
}
function AsideView({ previewSchema }){
  return (
    <div className="card-aside-view">
      {
        (panelView == 'preview'   ) &&  (<Carousel cards={ previewSchema } />)
...
/**
 * Carousel - Pré-visualização do formulário que está sendo preparado.
 * 

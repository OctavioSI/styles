  /*******************************************************************
  * Carousel Padrão Flows - POC com Flowable
  * 
  * STATUS: Em andamento
  * 
  * CHANGELOG
  ...
  *   
  * BUG FIXES
  *   - Card conditions: Cards com condições não estavam funcionando  
  *   - Definitions: Arrumado problema com definitions nos schemas (validation)
  *
  * KNOWN ISSUES
  *
  ...
  ********************************************************************/
  (function () {
    ...
    ********************************************************************/
    ...
    useEffect(() => {
      async function buscarCustomField(idTask, tipo, domain, env) {
        let config = {
          method: "POST",
          url: "https://actions.looplex.com/api/code/E0B98370-4C21-11EE-A76C-2D16796F3688",
          data: {
            command: "addCustomField",
            server: env,
            domain: domain,
            idCompromisso: idTask,
            tipoItemAgenda: tipo,
            customFields: [],
            client_id: "flows",
          },
        ...
        return res.data.output.Custom_Fields.flows;
      }

      let { idTask, tipo, domain, env } = props.embeddedData;

      buscarCustomField(idTask, tipo, domain, env)
        .then((customFields) => {
          let formConfig = customFields.formConfig;

          const initialform = {
            id: formConfig.initialformId,
            tenant: formConfig.initialformTenant ? formConfig.initialformTenant : 'looplex.com.br',
            document: formConfig.initialformDocument ? formConfig.initialformDocument : 'teste001',
            base_filename: formConfig.base_filename ? formConfig.base_filename : 'file.docx',
            formTitle: formConfig.formTitle ? formConfig.formTitle : "Form Looplex",
            template: formConfig.template,
            author: formConfig.author ? formConfig.author : 'Looplex',
            language: formConfig.language ? formConfig.language : 'pt_br'
          };

          // console.log(JSON.stringify(initialform))
          setInitialForm(initialform)
          setFlowConfig(customFields.flowConfig)
        })
    ...
    function Row(props) {
      ...
      let isArray = Array.isArray(priorValue) || Array.isArray(currentValue)
      let row;
      if(isArray){
        row =
          <tr className={priorValue == currentValue ? "table-default" : "table-primary"}>
            ...
            <td style={{ wordBreak: "break-all", minWidth: "200px" }} className="col-xs-4" colspan="1">ANTES</td>
            <td style={{ wordBreak: "break-all", minWidth: "200px" }} className="col-xs-4" colspan="1">DEPOIS</td>
          </tr>
      } else {
        row =
          <tr className={priorValue == currentValue ? "table-default" : "table-primary"}>
    ...
    function Summary(props) {
      function tableBuilder(schema, previewFormData = {}, currentFormData = {}, sublevel = 0, arraykey = '') {
        let items = []
        if(schema.hasOwnProperty('properties')){
          for (let i = 0; i < Object.keys(schema.properties).length; i++) {
            ...
            if (type == 'array') {
              let row = <TableRow rows={tableBuilder(formItem, previewFormData[key] ? previewFormData[key] : {}, currentFormData[key] ? currentFormData[key] : {}, sublevel + 1, key)} subtitle={formItem.title} sublevel={sublevel + 1} />
              items.push(row)
            }
          }
        }else{ // Vim de um array, não de um objeto (na recursão)
          let isArray = Array.isArray(schema.items);
          if(isArray){
            // fixedItems -- nao estamos usando
          }else{ // Não é um Array, mas um object (apenas 1 item de array) -- mas o formData é sempre um array de valores
              let formItem = schema.items
              let type = formItem.type
              if (type == 'string' || type == 'integer') {
                let row = <Row title={formItem.title ? formItem.title : ""} priorValue={previewFormData[arraykey] ? previewFormData[arraykey] : []} currentValue={currentFormData[arraykey] ? currentFormData[arraykey] : []} />
                items.push(row)
              }
              if (type == 'object') {
                let row = <TableRow rows={tableBuilder(formItem, previewFormData[arraykey] ? previewFormData[arraykey] : {}, currentFormData[arraykey] ? currentFormData[arraykey] : {}, sublevel + 1)} subtitle={formItem.title} sublevel={sublevel + 1} />
                items.push(row)
              }
              /*
              if (type == 'array') {
                let indice = 0;
                //loop verificar qual mudou
                let row = <TableRow rows={tableBuilder(formItem, previewFormData[arraykey] ? previewFormData[arraykey] : {}, currentFormData[arraykey] ? currentFormData[arraykey] : {}, sublevel + 1, indice)} subtitle={formItem.title} sublevel={sublevel + 1} />
                items.push(row)
              }
              */
          }

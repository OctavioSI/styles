...
*
* v. 1.3
*   - Modularização:  Alterado o código do Formulario para modularização do Carousel
*
* v. 1.2
  ...
  useEffect(() => {
    ...
    const fetchInitialForm = async () => {
      ...
      if (res.data && res.data.output) {
        ...
        setAllLoadedCards(initialSchema)
        ...
        setIsLoading(false)
        // console.log('trigger Preloaded_cards', preloaded_cards)
        ...
        if (res.data.output.loginRequired && !isObjectEmpty(res.data.output.loginAccess)) { // Para acessar esse form é necessário um login antes
            ...
            if (!initialform.document || initialform.document == '') {
              ...
              for (let countAttemptsDoc = 0; countAttemptsDoc < maxAttempts; countAttemptsDoc++) {
                if (isLoadingInitialDocument.current) {
                  ...
                  console.log('mounting document');
                  isLoadingInitialDocument.current = true;
                  setIsLoadingDocumentDetails(true)
                  fetchDocumentDetails()
                    .then(res => {
                      console.log('Document details loaded successfully. Attempt: ' + countAttemptsDoc)
